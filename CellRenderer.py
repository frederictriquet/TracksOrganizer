...
class Styler:
    ...
    def display(self, data, _):
        return str(data)

    def tooltip(self, _, __):
        return None

    def foreground(self, _, __):
        return QtGui.QColor(QtCore.Qt.GlobalColor.white)

    def background(self, _, __):
        return QtGui.QColor(QtCore.Qt.GlobalColor.black)

    def decoration(self, _, __):
        return None

    def alignment(self, _, __):
        return None
...
class GenreStyler(Styler):
    def tooltip(self, data, track):
        return " - ".join(
            sorted(map(lambda genre: Conf.get_genre(genre), track["genre"]))
        )
